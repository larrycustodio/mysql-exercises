...
const expectReturnedColumns = (expectedColumns, results) => {
  ...
  expect(results instanceof Array).to.equal(
    true,
    'Exercise has not been started'
  );
  expect(results.every(r => Object.keys(r).length)).to.equal(true);

  expectedColumns.forEach((col, i) => {
    results.forEach((result, j) =>
      expect(Object.keys(result)[i]).to.equal(
        col,
        `Expected ${col} to be column #${i + 1} in result set.`
      )
    );
  });
};
...
describe('DML Exercises', function() {
  this.timeout(6500);
  ...
  it('01-select-statement.sql', () =>
    readAndExecute('01-select-statement').then(results => {
      expectReturnedColumns(allActorColumns, results[0]);
      expect(results[0].length).to.equal(
        200,
        'Expected to see a result set containing all 200 actors.'
      );

      expectReturnedColumns(['last_name'], results[1]);
      expect(results[1].length).to.equal(200);

      expectReturnedColumns(
        [
          'title',
          'description',
          'rental_duration',
  ...
  it('02-distinct.sql', () =>
    readAndExecute('02-distinct').then(results => {
      expectReturnedColumns(['last_name'], results);
      expect(results.length).to.equal(
        121,
        'Expected to see a result set containing all 121 last names.'
      );
    }));

  it('03-where.sql', () =>
    readAndExecute('03-where').then(results => {
      expectReturnedColumns(
      ...
      expect(
        results[1].every(r =>
          moment(r.payment_date).isAfter(moment('2005-05-27'))
        )
      ).to.equal(true);
    }));

  it('04-and.sql', () =>
    readAndExecute('04-and').then(results => {
      expectReturnedColumns(
        ['payment_id', 'amount', 'payment_date'],
        results[0]
      );
      ...
      expect(
        results[0].every(result =>
          moment(result.payment_date).isAfter(moment('2005-05-27 00:00:00'))
        )
      ).to.equal(true);
      expect(
        results[0].every(result =>
          moment(result.payment_date).isBefore(moment('2005-05-27 23:59:59'))
        )
      ).to.equal(true);
  ...
  it('06-in.sql', () =>
    readAndExecute('06-in').then(results => {
      expectReturnedColumns(['phone', 'district'], results[0]);
      expect(results[0].length).to.equal(24);
      expect(
        results[0].every(r =>
          ['California', 'England', 'Taipei', 'West Java'].includes(r.district)
        )
      ).to.equal(true);
      ...
      expect(
        results[1].every(r =>
          ['2005-05-25', '2005-05-27', '2005-05-29'].includes(
            moment(r.payment_date).format('YYYY-MM-DD')
          )
        )
      ).to.equal(true);
    }));
  ...
  it('07-between.sql', () =>
    readAndExecute('07-between').then(results => {
      expectReturnedColumns(allPaymentColumns, results[0]);
      expect(
        results[0].every(r =>
          moment(r.payment_date).isBetween(
            moment('2005-05-25 00:00:00'),
            moment('2005-05-26 23:59:59')
          )
        )
      ).to.equal(true);

      expectReturnedColumns(
        ['title', 'description', 'release_year', 'total_rental_cost'],
        results[1]
      );
      expect(
        results[1].every(
          r =>
            r.description.split(' ').length >= 18 &&
            r.description.split(' ').length <= 20
        )
      ).to.equal(true);
    }));

  it('08-like.sql', () =>
    readAndExecute('08-like').then(results => {
      expectReturnedColumns(
        ['title', 'description', 'release_year'],
        results[0]
      );
      expect(
        results[0].every(r => r.description.substring(0, 12) === 'A Thoughtful')
      ).to.equal(true);
  ...
  it('09-limit.sql', () =>
    readAndExecute('09-limit').then(results => {
      expectReturnedColumns(allPaymentColumns, results[0]);
  ...
  it('10-is-it-null.sql', () =>
    readAndExecute('10-is-it-null').then(results => {
  ...
  it('11-order-by.sql', () =>
    readAndExecute('11-order-by').then(results => {
      ...
      expect(results[1].map(r => r.rating)).to.deep.equal([
        'NC-17',
        'R',
        'PG-13',
        'PG',
      ...
      expect(
        results[3].every(
          r =>
            r.special_features.includes('Behind the Scenes') &&
            r.length < 120 &&
            r.rental_duration >= 5 &&
            r.rental_duration <= 7
        )
  ...
  it('12-inner-join.sql', () =>
    readAndExecute('12-inner-join').then(results => {
      expectReturnedColumns(
      ...
      results.forEach(r =>
        expect(moment.utc(r.rental_date).format('MM/DD/YYYY')).to.equal(
          '05/25/2005'
        )
      );
    }));
  ...
  it('13-left-join.sql', () =>
    readAndExecute('13-left-join').then(results => {
      expectReturnedColumns(
        ['customer_name', 'payment_date', 'amount', 'customer_active'],
        results
      );
  ...
  it('14-group-by.sql', () =>
    readAndExecute('14-group-by').then(results => {
  ...
  it('15-having.sql', () =>
    readAndExecute('15-having').then(results => {
      expectReturnedColumns(['customer_name', 'total_revenue'], results);
      expect(results.length).to.equal(5);
      expect(
        results.every(r => r.customer_name[0] === 'S' && r.total_revenue > 150)
      ).to.equal(true);
      expect(results).to.be.sortedBy('total_revenue', true);
    }));
  ...
  it('16-insert.sql', () => {
    ...
    return executeSql('select count(*) from city')
      .then(results => {
        beforeLength = results[0]['count(*)'];

        return readAndExecute('16-insert');
      })
      .then(() => executeSql('select count(*) from city'))
      .then(results => {
        afterLength = results[0]['count(*)'];

        expect(afterLength - beforeLength).to.equal(1);

        return executeSql('delete from city where city = "San Diego"');
      })
  });
  ...
  it('17-update.sql', () => {
    ...
    return executeSql(sql)
      .then(() => readAndExecute('17-update'))
      .then(() => executeSql('select description from film where film_id = 100'))
      .then(results => {
        expect(results.length).to.be.gt(0);
        expect(results[0].description).to.not.equal(
  ...
  it('18-delete.sql', () =>
    resetDb()
      .then(() => readAndExecute('18-delete'))
      .then(results => executeSql('select count(*) from film'))
      .then(results => expect(results[0]['count(*)']).to.equal(958)));
